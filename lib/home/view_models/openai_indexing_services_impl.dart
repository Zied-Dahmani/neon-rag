...
class OpenAIIndexingServicesImpl extends OpenAIIndexingServices {
  ...
  Future<List<List<double>>> getEmbeddings(List<List<String>> chunks) async {
    debugPrint("Embedding with Cohere...");
    ...
    for (var chunk in chunks) {
      final response = await client.post(
        ...
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer ${dotenv.env['COHERE_API_KEY']!}',
        },
        body: jsonEncode({
      ...
      if (response.statusCode == 200) {
        final Map<String, dynamic> responseData = jsonDecode(response.body);
        List<dynamic> embedding = responseData['embeddings'][0];
  ...
  Future<String> queryNeonTable(String tableName, String query) async {
    ...
    print("Query Embedding: $embedQuery");
    ...
    debugPrint("Raw DB results: $getSimilar");

    if (getSimilar.isEmpty) {
      debugPrint("No results found in database.");
    }
    ...
    if (csvMetadata.isNotEmpty) {
      ...
      debugPrint("Concatenated page content: $concatPageContent");
      ...
      print("Query Received: $finalResponse");
